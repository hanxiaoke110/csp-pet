...
function conditionMet(item: WardrobeItem, cardCount: number): boolean {
  ...
  if (item.acquisition.rule === 'dungeon-crystal') {
    try {
      const player = JSON.parse(localStorage.getItem('dungeon_player') || '{}');
      return player?.trialInventory?.ownedCosmetics?.includes('frame-crystal') === true;
    } catch { return false; }
  }
  return false;
}
...
function WardrobeCard({ item, owned, equipped, previewing, cardCount, onToast, onPreview }: {
  ...
  const canClaim = conditionMet(item, cardCount);
...
function WardrobePanel({ onToast }: { onToast: (text: string) => void }) {
  ...
  useEffect(() => {
    for (const item of catalog.items) {
      ...
      if (item.acquisition.type === 'condition' && conditionMet(item, cardCount)) grant(item.id);
    }
...
export default function MyPage() {
  ...
  const todayTasks = useMemo(() => [
    { text: '完成一次学习或练习', done: unlockedCount > 0 },
    { text: '照顾一位灵犀智子', done: ownedPets.some(p => (p.hunger || 0) >= 70) },
    { text: '写下今天的一句话', done: Boolean(journal[today]?.trim()) },
  ], [journal, ownedPets, today, unlockedCount]);
