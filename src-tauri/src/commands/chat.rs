...
#[tauri::command]
pub fn add_chat_message(db: State<Database>, session_id: String, role: String, content: String) -> Result<(), String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    conn.execute(
...
#[tauri::command]
pub fn get_chat_messages(db: State<Database>, session_id: String) -> Result<Vec<ChatMessage>, String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare(
        "SELECT id, session_id, role, content, created_at FROM chat_messages WHERE session_id = ?1 ORDER BY id ASC"
    ).map_err(|e| e.to_string())?;
    let rows = stmt.query_map([&session_id], |row| Ok(ChatMessage {
        id: Some(row.get(0)?), session_id: row.get(1)?, role: row.get(2)?,
        content: row.get(3)?, created_at: Some(row.get(4)?),
    })).map_err(|e| e.to_string())?;
