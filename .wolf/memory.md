...
- 追加确认与修复（v1.7.28 已取消，改发 1.7.29）：用户确认 C4 阶段保留（后续上线）；超级挑战 3/5、4/5 改为按正确率 ≥60%/≥80% 判定；阶段成就按课程阶段 25/50/68 判定（C4 保持 100）；指南补“多智子”说明（伴生槽 2500/5000g + 硬件免责的趣味提醒）、心情/好感度具体数值、成就奖励含改名卡明细。改名卡发放逻辑核对正常（addRenameCards 存在且会 save，之前不触发是因为成就本身没解锁）。

## 2026-08-03 — v1.7.29 发版记录

- 内容：成就修复（周常完美重复计数、超级挑战按正确率判定、阶段成就按课程阶段、三连完美描述）+ 饲养指南全面更新（含多智子硬件温馨提示）+ 此前未发的所有客户端改动。
...
- 过程：版本三处同步（package.json/tauri.conf.json/App.tsx）+ package-lock 顺手同步（此前停在 1.7.36）+ vitest 160/160 + tsc + build + build:dungeon + 题库 86/86 → commit 1639de8 + tag v1.7.38 → push 双远端。CI 三平台构建全 success（约 12 分钟），**release job 启动即取消**（用户定调：不再等 30 分钟）→ 手动补传：下载→改名→签名→验签（signer 写的 .sig 是 base64 包装，需 base64 -d -i 解码后再 minisign -Vm）→ Gitee v1.7.38 release 已由 release job 建好（含源码包），attach_files 三包 201 → update.json 生成并 git push 双远端 + API 验证=1.7.38 → 删 v1.7.36 旧 release（保留 1.7.37/38）→ GitHub Release 备份 → 公告 id 27 置顶 → 飞书文档文字同步 → 用户确认后 `--sync-attachments` 附件同步完成（exe 22 chunks + aarch64 24 chunks + x64 24 chunks，三包全部换新为 1.7.38）。
