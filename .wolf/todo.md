...
- [ ] **通关榜少算修复（2026-08-08，已提交 94fa578，worker 未部署）**：服务端 sync 通关状态修复通道（只升不降）+ 客户端启动修复推送。**⚠️ 待办：`npx wrangler login` 过期需重新登录后 `npx wrangler deploy`（worker=api）才生效**；部署后学生重打一遍各副本 Boss 即可修复榜单（当前客户端即可），下个客户端版本启动自动修复。
